...
| 421 cortados contando como «tiene equipo» | `scripts/fantasy_build.py`, `fantasy/roster_status.py` | `mark_rostered` preguntaba «¿aparece en el fichero de plantillas?» y el fichero trae TAMBIÉN a los cortados, los retirados y los de listas de reserva. En el board de 2026 eran 73 filas leyéndose como un jugador normal la víspera de un draft — Tyreek Hill entre ellas. Ahora `rostered` significa «tiene equipo NFL» y la situación fina (activo / reserva / prácticas / exento) es una capa propia con la fecha de su fichero. Marca, no calcula: el prefijo `roster_` está para poder comprobarlo leyendo la lista de campos |
