...
| El registro de capacidades no lo lee NADIE en la web | `src/oracle/capabilities.py`, `web/app` | Las 34 capacidades se exportan al payload y `app/` tiene **cero** referencias a ellas: cada frontera de autoridad de la interfaz es prosa escrita a mano que puede desviarse del registro sin que falle nada. Es la misma deriva que las cifras de portada, en las afirmaciones de autoridad |
